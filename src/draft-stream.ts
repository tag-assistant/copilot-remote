/**
 * Telegram Draft Stream Engine
 *
 * Streams text to Telegram chats via draft previews or message edits,
 * with throttled flushing, generation-based stale detection,
 * and draft-to-message materialization.
 *
 * Ported from OpenClaw (MIT License).
 * https://github.com/austenstone/openclaw
 */
...
export function createTelegramDraftStream(params: {
  ...
  // In copilot-remote, default to draft transport when available (all chats are DM-like).
  const prefersDraftTransport =
    requestedPreviewTransport === "draft"
      ? true
      : requestedPreviewTransport === "message"
        ? false
        : true; // auto → prefer draft
  ...
  const sendDraftTransportPreview = async ({
    ...
    await resolvedDraftApi!(
      ...
      draftId,
      renderedText,
      Object.keys(draftParams).length > 0 ? (draftParams as { message_thread_id?: number; parse_mode?: "HTML" }) : undefined,
    );
    return true;
  };
  ...
  const materialize = async (): Promise<number | undefined> => {
    ...
    try {
      ...
      if (typeof sentId === "number" && Number.isFinite(sentId)) {
        ...
        // Clear the draft so Telegram's input area doesn't show stale copy.
        if (resolvedDraftApi != null && streamDraftId != null) {
          ...
          const clearThreadParams =
            usedThreadParams && threadParams?.message_thread_id != null
              ? { message_thread_id: threadParams.message_thread_id as number }
              : undefined;
          try {
